...
[csv 데이터](https://drive.google.com/open?id=1YhUlD-s_2eDFOMbIUySuVBqjZOplYVxu) 링크

[excel 데이터](https://drive.google.com/open?id=1aZIZrTF3vdtT8iv_f-k-C9ZWi07FC1a1) 링크
...
mydb 
<
- dbConnect(SQLite(), "testDB.sqlite")
...
rs 
<
- dbSendQuery(mydb, 'SELECT * FROM mtcars')
...
rs 
<
- dbSendQuery(mydb, 'SELECT * FROM iris WHERE "Sepal.Length" 
<
 :x')
...
# SQL 구문 실행하기
dbExecute(mydb, 'DELETE FROM iris WHERE "Sepal.Length" 
<
 4')
rs 
<
- dbSendStatement(mydb, 'DELETE FROM iris WHERE "Sepal.Length" 
<
 :x')
dbBind(rs, param = list(x = 4.5))
...
con 
<
- odbcConnect("DSN", uid = "userid", pw="password")

# DB의 TABLE 읽어오기
df 
<
- sqlFetch(con, sqtable = "TABLENAME")
...
query 
<
- paste0(" SELECT TOP 1000 [Country]
                                ,[keywordType]
                                ,[subKeywordType]
                                ,[keyword]
